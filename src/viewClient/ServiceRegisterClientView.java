...
public class ServiceRegisterClientView extends BorderPane implements ExceptionAndEventHandler{

	private ConnectionMaster 		 connection;
	private ExceptionAndEventHandler parent;	
	private ServiceRegisterView 		 		 service;

	/**
	 * This is the default constructor
	 */
	public ServiceRegisterClientView( ExceptionAndEventHandler parent, ServiceRegisterView service) {
		super();
		this.parent = parent;
	...
	private ServiceRegisterView getService(){
		return this.service;
	}
	...
	protected void setDetailsTable( Anything object) {
		...
		if (object == null && this.getConnection() != null) object = this.getConnection().getServiceRegisterView();
		if (object == null){
	...
	public ServiceRegisterConnection getConnection(){
        	return (ServiceRegisterConnection)this.connection;
	}
	...
	public void handleRefresh(){
		Platform.runLater(new Runnable() {
			public void run(){
				try {
					getNavigationTree().refreshTree();
					java.util.Vector<?> errors = getConnection().getServiceRegisterView().getErrors();
	...
	**/
	public void initializeConnection(){
		Platform.runLater( new  Runnable() {
			public void run() {
				getNavigationTree().setModel((TreeModel) getConnection().getServiceRegisterView());	
    ...
    private abstract class ServiceRegisterMenuItem extends MenuItem{
        private ServiceRegisterMenuItem(){
            this.setGraphic(getIconForMenuItem(this));
        }
        abstract protected ImageView accept(MenuItemVisitor visitor);
    }
    private class RegisterPRMTRStringPRMTRStringPRMTRMenuItem extends ServiceRegisterMenuItem{
    ...
    private java.util.Vector<javafx.scene.control.Button> getToolButtonsForStaticOperations() {
        ...
        currentButton.setOnAction(new EventHandler<ActionEvent>(){
            public void handle(javafx.event.ActionEvent e) {
                final ServiceRegisterRegisterStringStringMssgWizard wizard = new ServiceRegisterRegisterStringStringMssgWizard("register");
    ...
    private ContextMenu getContextMenu(final ViewRoot selected, final boolean withStaticOperations, final Point2D menuPos) {
        ...
        if (selected != null){
            try {
                this.setPreCalculatedFilters(this.getConnection().serviceRegister_Menu_Filter((Anything)selected));
	...
	class ServiceRegisterRegisterStringStringMssgWizard extends Wizard {

		protected ServiceRegisterRegisterStringStringMssgWizard(String operationName){
			super(ServiceRegisterClientView.this);
			getOkButton().setText(operationName);
			getOkButton().setGraphic(new RegisterPRMTRStringPRMTRStringPRMTRMenuItem ().getGraphic());
		}
		protected void initialize(){
			this.helpFileName = "ServiceRegisterRegisterStringStringMssgWizard.help";
			super.initialize();		
		}
		...
		protected void addParameters(){
			getParametersPanel().getChildren().add(new StringSelectionPanel("username", this));
			getParametersPanel().getChildren().add(new StringSelectionPanel("passwort", this));		
		}	
	...
	private ImageView getIconForMenuItem(ServiceRegisterMenuItem menuItem){
