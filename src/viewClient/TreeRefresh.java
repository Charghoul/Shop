...
//			System.out.println("root: " + result.size() + "f�r:  " +rootFX.getValue().getWrappedObject().toString());
		} else {
			for (TreeItem<ViewObjectInTree> child : rootFX.getChildren()) {
				result.addAll( this.getMatchingItems( child, searchString) );				
//				System.out.println( result.size() +  "  f�r:  " +child.getValue().getWrappedObject().toString());
			}
		}
		return result;		
	}
